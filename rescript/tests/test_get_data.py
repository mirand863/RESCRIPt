...
from rescript.get_data import (_assemble_silva_data_urls,
                               ...
                               _assemble_gtdb_data_urls)
from rescript.plugin_setup import _SILVA_VERSIONS, _SILVA_TARGETS, _GTDB_VERSIONS, _GTDB_DOMAINS
...
class TestGetGTDB(TestPluginBase):
    ...
    def test_assemble_gtdb_data_urls(self):
        for version in _GTDB_VERSIONS:
            for domain in _GTDB_DOMAINS:
                urls = _assemble_gtdb_data_urls(version, domain)
                # Validate URLs
                for _, url, _ in urls:
                    try:
                        urlopen(url)
